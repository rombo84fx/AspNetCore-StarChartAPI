    ...
    public class ConfigureMiddlewareTests
    {
        ...
        public void CallAddApplicationDbContextInConfigureServicesTest()
        {
            ...
            Assert.True(rgx.IsMatch(file),
                "`Startup.ConfigureServices` didn't contain a call for `AddDbContext` on `services` with the type argument `ApplicationDbContext`");
            ...
            Assert.True(file.Contains("UseInMemoryDatabase"),
                @"`Startup.ConfigureServices` didn't provide `options => options.UseInMemoryDatabase(""StarChart"")` as an argument for `AddDbContext<ApplicationDbContext>`");
        }
        ...
        public void CallAddMvcInConfigureServicesTest()
        {
            ...
            Assert.True(rgx.IsMatch(file),
                "`Startup.ConfigureServices` didn't contain a call for `AddMvc` on `services`");
        }
